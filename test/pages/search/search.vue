				...
				<view v-for="(item,index) in msg" :key='index'>
					<view class="main" @click="navigator(item.coupon_share_url)">
						...
						<view class="main-main">
							...
							<text style="padding-left: 12px;">销量:{{item.volume}}</text>
			...
			getHomePage(){
				...
				uni.request({
					...
					success:(res) => {
						uni.hideLoading()
						if(res.statusCode==200){
							if(res.data.tbk_dg_material_optional_response.result_list.map_data.length>0){
													
								this.msg = this.msg.concat(res.data.tbk_dg_material_optional_response.result_list.map_data)
